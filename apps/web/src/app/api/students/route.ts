...
export async function GET() {
  try {
    ...
    return jsonError("Không thể tải danh sách học sinh.", 500, (error as Error).message);
  }
}
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return jsonError("Dữ liệu học sinh không hợp lệ.", 400, formatZodError(parsed.error));
  }
  ...
  try {
    ...
    if (getPgErrorCode(error) === "23503") {
      return jsonError("parent_id không hợp lệ.", 400);
    }

    return jsonError("Không thể tạo học sinh.", 500, (error as Error).message);
  }
}
